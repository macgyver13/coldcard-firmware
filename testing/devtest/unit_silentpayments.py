...
from public_constants import (
    ...
    PSBT_IN_SP_PARTIAL_ECDH_SHARE, 
    PSBT_IN_SP_PARTIAL_DLEQ,
)
...
assert PSBT_IN_SP_PARTIAL_ECDH_SHARE == 0x21
assert PSBT_IN_SP_PARTIAL_DLEQ == 0x22
...
class MockInput:
    def __init__(self):
        ...
        self.sp_partial_ecdh_shares = None
        self.sp_partial_dleq_proofs = None
        self.musig_pubkeys = []
...
assert psbt._compute_and_store_proof_entries(sv) is True
...
# Idempotent
psbt._compute_and_store_proof_entries(sv)
assert len(psbt.sp_global_ecdh_shares) == 1
...
outp.sp_v0_info = TEST_SCAN_KEY + TEST_SPEND_KEY
psbt.outputs = [outp]
psbt._compute_and_store_proof_entries(
    {
...
outp.sp_v0_info = TEST_SCAN_KEY + TEST_SPEND_KEY
psbt.outputs = [outp]
psbt._compute_and_store_proof_entries({TEST_DERIV_OURS: TEST_PRIVKEY})
# No signable inputs
psbt = MockPSBT()
...
outp.sp_v0_info = TEST_SCAN_KEY + TEST_SPEND_KEY
psbt.outputs = [outp]
assert psbt._compute_and_store_proof_entries({}) is False
