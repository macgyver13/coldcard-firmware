...
from constants import (ADDR_STYLES, ADDR_STYLES_SINGLE, SIGHASH_MAP, simulator_fixed_xfp,
                       simulator_fixed_tprv, SIGHASH_MAP_NON_TAPROOT)
from txn import *
...
def test_bip143_attack(try_sign, sim_exec, fake_txn, settings_get):
    ...
    mod1 = fake_txn([
        ["p2wpkh", None, 500001000],
        ["p2wpkh", None, 2000000000],
    ], [["p2wpkh", 2500000000]], fee=0)
...
@pytest.fixture
def history_data(settings_get):
    def doit():
        return settings_get('ovc') or []
    return doit
...
def test_bip143_attack_data_capture(num_utxo, try_sign, fake_txn, press_cancel,
    ...
    curr = history_data()
    assert len(curr) == min(128, num_utxo)

    t = CTransaction()
    t.deserialize(BytesIO(txn))
    assert t.txid().hex() == txid

    # expect all of new segwit "change outputs" to be recorded (none of the
    # non-segwit change), plus the one input we signed
    # plus the one input we "revealed"
    after1 = settings_get('ovc')
    assert len(after1) == min(128, num_utxo)

    all_utxo = history_data()
    assert len(all_utxo) == min(128, num_utxo)
...
def test_ovc_not_poisoned_pre_approval(try_sign, fake_txn, settings_get, sim_exec,
                                       history_data):
    # A parsed-but-cancelled PSBT must NOT persist first-seen UTXO amounts to flash;
    # only an actual signature may commit them.
    sim_exec('import history; history.OutptValueCache.clear()')
    assert len(history_data()) == 0
    assert not settings_get('ovc')

    honest = fake_txn(1, 1, input_amount=int(0.49995 * 1E8))
...
def test_ovc_commit_only_signed_inputs(try_sign, fake_txn, settings_get, sim_exec,
                                       history_data):
    ...
    try_sign(fake_txn(2, 1), accept=True, finalize=True)
...
def test_ovc_multi_input_amounts(try_sign, fake_txn, settings_get, sim_exec):
    ...
    sim_exec('import history; history.OutptValueCache.clear()')
    psbt = fake_txn([
        ["p2wpkh", None, 100000000],
        ["p2wpkh", None, 125000000],
    ], [["p2wpkh", 224990000]], fee=10000)
...
def test_ovc_singlesig_change_capture(in_style, try_sign, fake_txn, settings_get,
    ...
    # Single-sig segwit change produced by a txn we sign must be captured into
    # the UTXO value cache at finalize time.
    sim_exec('import history; history.OutptValueCache.clear()')
    assert len(history_data()) == 0

    psbt_bytes = fake_txn(
        [[in_style]],
        [["p2wpkh", None, False], [in_style, None, True]],
        fee=10000)
    psbt = BasicPSBT().parse(psbt_bytes)

    _, tx = try_sign(psbt_bytes, accept=True, finalize=True, exit_export_loop=False)
    txid = txid_from_export_prompt()
    press_cancel()
    press_cancel()
...
def test_p2sh_p2wpkh_multiple_bip32_paths_rejected(fake_txn, start_sign, cap_story):
    psbt = fake_txn(1, 1, addr_fmt="p2sh-p2wpkh")
...
def test_nested_segwit_witness_utxo_only_fee_shown(dev, fake_txn, start_sign,
                                                  cap_story, end_sign):
    psbt = fake_txn(1, 2, dev.master_xpub, addr_fmt="p2sh-p2wpkh")
    start_sign(psbt)
    time.sleep(.1)
    title, story = cap_story()
    assert title == "OK TO SEND?"
...
def test_own_legacy_witness_utxo_only_fails(dev, fake_txn, start_sign, cap_story, press_cancel):
    ...
    psbt = fake_txn(1, 2, dev.master_xpub, addr_fmt="p2pkh", psbt_hacker=hack)
    start_sign(psbt)
    time.sleep(.1)
    title, story = cap_story()
    assert title == "Failure"
...
def test_foreign_legacy_witness_utxo_only_ok(dev, fake_txn, start_sign, cap_story, end_sign):
    ...
    psbt = fake_txn(2, 2, dev.master_xpub, addr_fmt="p2pkh", psbt_hacker=hack)
    start_sign(psbt)
    time.sleep(.1)
    title, story = cap_story()
    assert title == "OK TO SEND?"
...
def test_op_return_trailing_data_not_hidden(fake_txn, start_sign, cap_story):
    ...
    def hack(psbt):
        txn = CTransaction()
        txn.deserialize(BytesIO(psbt.txn))
        txn.vout[0].scriptPubKey = weird
        psbt.txn = txn.serialize_with_witness()

    psbt = fake_txn(1, 2, addr_fmt="p2wpkh", psbt_v2=False, psbt_hacker=hack)
    start_sign(psbt)
    time.sleep(.1)
    title, story = cap_story()
    assert title == "OK TO SEND?"
...
def test_txout_explorer_qr_too_big_single_item(fake_txn, start_sign, cap_story,
                                               cap_screen, need_keypress,
                                               pick_menu_item, press_cancel, is_q1):
    if not is_q1:
        raise pytest.skip("Q1 QR fallback")

    outputs = [["p2wpkh", 9_999_980] for _ in range(10)]
    outputs.append(["op_return", 0, None, b"a" * 1000])
    psbt = fake_txn(1, outputs, addr_fmt="p2wpkh", psbt_v2=False)
    start_sign(psbt)
    time.sleep(.1)
    title, _ = cap_story()
    assert title == "OK TO SEND?"
...
def test_tx_explorer_goto_idx_single_item_yikes(mi, fake_txn, start_sign,
                                                cap_story, use_testnet,
                                                need_keypress, pick_menu_item,
                                                press_cancel, cap_menu):
    use_testnet()
    psbt = fake_txn(1, 1, addr_fmt="p2wpkh")
    start_sign(psbt)
    title, _ = cap_story()
    assert title == "OK TO SEND?"
    ...
    need_keypress("2")  # must not yikes on a single-item explorer
    press_cancel()
    menu = cap_menu()
...
def test_input_explorer_foreign_bad_sighash(fake_txn, start_sign, cap_story,
                                            need_keypress, pick_menu_item,
                                            press_cancel, use_testnet, goto_home):
    use_testnet()

    def hack(psbt):
        # Input zero is foreign, but still carries a non-standard sighash value.
        foreign_xfp = b"\xab\xcd\xef\x01"
        psbt.inputs[0].bip32_paths = {
            pk: foreign_xfp + path_bytes[4:]
...
def test_fake_txn_spend_p2pk(mode, finalize, fake_txn, start_sign, end_sign, cap_story):
    ...
    outputs = [[style, None, True], ['p2pkh']]
    psbt = fake_txn(1, outputs, p2pk_in=mode)
...
def test_p2pk_change_output_renders(mode, change, fake_txn, start_sign, cap_story, dev,
    ...
    outputs = [['p2pkh', 1_000_000], ['p2pkh', 98_990_000]]
    psbt = fake_txn(1, outputs, p2pk_in=mode, psbt_v2=False, psbt_hacker=hack)
    goto_home()
    start_sign(psbt)
    time.sleep(.1)
    title, story = cap_story()
