...
class SilentPaymentsMixin:
    ...
    def process_silent_payment_outputs(self, sv):
        ...
        self._compute_and_store_proof_entries(sv)
        ...
        for inp in self.inputs:
            if self._is_musig_sp_input(inp):
                ...
                for participant_pk in participants:
                    for scan_key in scan_keys:
                        if (
                            not inp.sp_partial_ecdh_shares
                            or (scan_key, participant_pk) not in inp.sp_partial_ecdh_shares
                        ):
                            return False
        return True
    ...
    def _validate_ecdh_coverage(self):
        ...
        for scan_key in scan_keys:
            ...
            has_musig = any(
                inp.is_musig
                and inp.sp_partial_ecdh_shares
                and any(share_scan_key == scan_key for (share_scan_key, _) in inp.sp_partial_ecdh_shares)
                for inp in self.inputs
            )
            ...
            if not has_global:
                for i, inp in enumerate(self.inputs):
                    ...
                    if not has_share and scan_key_has_script:
        ...
        for i, inp in enumerate(self.inputs):
            if self._is_musig_sp_input(inp) and inp.sp_partial_ecdh_shares:
                for (scan_key, participant_pk), share in inp.sp_partial_ecdh_shares.items():
                    if (
                        not inp.sp_partial_dleq_proofs
                        or (scan_key, participant_pk) not in inp.sp_partial_dleq_proofs
                    ):
                        raise FatalPSBTIssue("MuSig2 share missing DLEQ for input #%d" % i)
                    proof = inp.sp_partial_dleq_proofs[(scan_key, participant_pk)]
    ...
    def _compute_and_store_proof_entries(self, sv):
        """
        Compute proof entries (ECDH shares and DLEQ proofs) for our inputs, store in PSBT fields
        ...
        for scan_key in scan_keys:
            if not has_foreign and not self.has_musig_sp_inputs():
                ...
                self._store_proof_entry(combined_sk, scan_key, self.sp_global_ecdh_shares, self.sp_global_dleq_proofs)
                ...
                for inp, input_sk in input_material:
                    ...
                    self._store_proof_entry(input_sk, scan_key, inp.sp_ecdh_shares, inp.sp_dleq_proofs)
                    del input_sk
        del input_material
        return True
    ...
    def _store_proof_entry(self, sk, scan_key, shares, proofs):
        """
        Store a proof entry (ECDH share and DLEQ proof) for scan key if not already present

        Raises FatalPSBTIssue if the existing entry is incomplete or its ECDH share is not from our key
        """
        if scan_key not in shares and scan_key not in proofs:
            shares[scan_key] = _compute_ecdh_share(sk, scan_key)
            proofs[scan_key] = generate_dleq_proof(sk, scan_key)
            return

        if scan_key not in shares or scan_key not in proofs:
            raise FatalPSBTIssue("Proof entry must have both ECDH share and DLEQ proof")
        if shares[scan_key] != _compute_ecdh_share(sk, scan_key):
            raise FatalPSBTIssue("Existing ECDH share does not match our key")
    ...
    def _contribute_musig_ecdh_shares(self, sv, input):
        ...
        scan_keys = self._get_silent_payment_scan_keys()
        for scan_key in scan_keys:
            if input.sp_partial_ecdh_shares and (scan_key, my_participant_pk) in input.sp_partial_ecdh_shares:
                contributed = True
                continue
            ...
            input.sp_partial_ecdh_shares = input.sp_partial_ecdh_shares or {}
            input.sp_partial_dleq_proofs = input.sp_partial_dleq_proofs or {}
            input.sp_partial_ecdh_shares[(scan_key, my_participant_pk)] = my_share
            input.sp_partial_dleq_proofs[(scan_key, my_participant_pk)] = my_proof
            contributed = True
    ...
    def _musig_input_ecdh_share(self, input, scan_key):
        ...
        """
        if not input.sp_partial_ecdh_shares:
            return None
        ...
        partial_shares = [
            ...
            for (entry_scan_key, participant_pk), share in input.sp_partial_ecdh_shares.items()
